...
promise_test(async testCase => {
  const file = await createFile(testCase, "file_length_large");
  const kOutOufInt64Bounds = 2**63+2**11;
  await promise_rejects_js(testCase, TypeError,
                           file.setLength(kOutOufInt64Bounds));
}, 'NativeIOFile.setLength() throws when attempting to set the length ' +
